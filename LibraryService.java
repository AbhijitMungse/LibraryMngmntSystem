...
public class LibraryService {
	...
	// this is bookissue details array of type BookIssueDetails class,will store
	// bookissue details
	...
	static void addBooks() {
		...
		book[index] = new BookDetails(bookId, bookName, bookAuthor, noOfBooks);
		index++;
			...
			for (int i = 0; i < index; i++) {
				if (book[i].getBookId() == id) {
					if (book[i].getNoOfBooks() > 0) {
						...
						bookIssue[issueindex] = new BookIssueDetails(issueId, book[i].getBookName(),
								book[i].getBookAuthor(), issueDate, returnDate);
						issueindex++;
						issueId++;
						break;
					}
